...
def get_tasks():
    def scrapeLoop(gazpacho):
        ...
        gazpacho = cleaner.clean_html(gazpacho)                    #TODO: see if more can be removed
        ...
        facebook = checkListExists(gazpacho.xpath("//p[@class='url-facebook']"))
        ...
        charityWebsite = checkListExists(gazpacho.xpath("//p[@class='url-web']"))
        imageURL = trimWebsite(imageURL)
        charityWebsite = trimWebsite(charityWebsite)
        addressLine1 = trimEdgeSpace(trimDoublePlusSpace(trimNewLine(addressLine1)))

        #something is removing quotes from string. Could be the checkDescExists function
        longDescription = trimEdgeSpace(trimDoublePlusSpace(trimNewLine(longDescription))) 
        
        #this method does not work for 'Mr. & Mrs.'
        shortDescription = longDescription[:longDescription.find(".")] + "..." if longDescription != "" else ""
    ...
    def threadLoop(soup):
        ...
        URLS = grequests.map((grequests.get(page) for page in charityPages), size=8)
    ...
    cleaner = Cleaner(links=False, style=True, safe_attrs_only=False, javascript=True)
    ...
    initialSearch = requests.get("https://www.charitychoice.co.uk/charities/search/?t=qsearch&q=all&onlinedonations=0&pid=1")
...
if __name__ == "__main__":
    ...
    ##TODO: check if base_url can avoid requests